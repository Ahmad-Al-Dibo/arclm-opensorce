...
| Llama, Mistral, Gemma, Falcon through Hugging Face | `owner/model-id` | Expected decoder-only causal LM | Compatible but not tested | Not verified | Generic path only | Loader may accept them; users must validate behavior. |
