...
The canonical package name is `arclm`; the project name used in user-facing text is ArcLM. The open-source edition keeps the framework simple and focused on production-oriented causal-language-model workflows with explicit support boundaries.
...
- Hidden model loading behavior or automatic `trust_remote_code` enablement.
- Broad, unrelated machine-learning utilities outside the data-to-causal-LM workflow.
