...
| `torch.save` / `torch.load` | Checkpoints/artifacts | REPLACE | Public persistence moves to `.arcmodel`, `.arcckpt`, `.arcadapter`; tensor bytes may use safe backend encodings. |
...
| Safetensors | Tensor serialization | KEEP AS LOW-LEVEL DEPENDENCY | Good candidate for internal tensor byte storage. ArcLM owns manifest/layout/integrity. |
...
2. Dataset engine: move sequence batching out of public `torch.utils.data`
   concepts.
3. Artifact engine: replace public checkpoint dictionaries with `.arcmodel`,
   then add `.arcckpt` and `.arcadapter`.
